...
| M6 — packaging | **Done.** `build.ps1 -Package` assembles the Thunderstore zip; never uploaded |
...
packaging/                  Thunderstore manifest, icon, README, changelog
```
...
- `-PluginsDir <path>` to install into an r2modman profile rather than the
  Steam folder.
- `-Package` to build the Thunderstore zip.
...
That writes `dist/ValheimRelay-<version>.zip`, ready to upload at
[thunderstore.io/c/valheim/create](https://thunderstore.io/c/valheim/create/):
`manifest.json`, `icon.png`, `README.md` and `CHANGELOG.md` at the root, with
the two assemblies under `plugins/`.

It has to happen on a machine with Valheim installed, and so cannot be a CI job:
the plugin only compiles against the game's own assemblies, which are never
committed or redistributed.

Before packaging it checks the two things Thunderstore only tells you about
after a failed upload — that the icon is exactly 256x256, and that the three
required files are present — plus one it would never tell you about at all: that
`manifest.json`, the csproj `<Version>` and `Plugin.PluginVersion` still agree.
Bump all three together.

`packaging/icon.png` is generated by `packaging/make-icon.py`, kept in the repo
so the icon can be edited rather than being an unexplained binary. It needs
